...
// You should have received a copy of the GPLv3 General Public License  along with this program; if not, write to the Free Software  Foundation, Inc., 59 Temple Place, Suite 330, Boston, MA 02111-1307,  USA.

namespace LaserGRBL.ComWrapper
{
    public class UsbSerial : IComWrapper
    {
        private MySerial com;
            ...
            if (!IsOpen)
            {
                ...
                try
                {
                    com = new MySerial
                    {
                        ...
                        NewLine = "\n",
                        WriteTimeout = 1000, //se si blocca in write

                        DtrEnable = Settings.GetObject("HardReset Grbl On Connect", false),
                        RtsEnable = Settings.GetObject("HardReset Grbl On Connect", false)
                    };
        ...
        public bool IsOpen => com != null && com.IsOpen;
    ...
    internal class MySerial : System.IO.Ports.SerialPort
    {
        protected override void Dispose(bool disposing)
        {
            try
            {
                var mytype = typeof(System.IO.Ports.SerialPort);
                var field = mytype.GetField("internalSerialStream", System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.NonPublic);

                if (field.GetValue(this) is System.IO.Stream stream)
                {
                    try { stream.Dispose(); }
                    catch { }
                }
